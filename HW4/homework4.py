...
def EDA():
    ...
    gamesdf["app_id"].value_counts
    gamesdf["title"].value_counts
    gamesdf["date_release"].value_counts
    gamesdf["win"].value_counts
    gamesdf["mac"].value_counts
    gamesdf["linux"].value_counts
    gamesdf["rating"].value_counts
    gamesdf["positive_ratio"].value_counts
    gamesdf["user_reviews"].value_counts
    gamesdf["price_final"].value_counts

    # Correlation Heatmap + raw printed
    df_numeric = gamesdf.select_dtypes(include=np.number)
    ...
    print("Unique values per feature:\n{}".format(df_numeric.nunique()))

    # Significant (more fraudsters from foreign requests)
    sns.barplot(x=gamesdf["user_reviews"], y=gamesdf["price_final"])
    plt.show()

    # Seeing how many positive ratios are within the user reiews
    sns.barplot(x=gamesdf["positive_ratio"], y=gamesdf["user_reviews"])
    plt.show()
    
    # Positive ratio compared to the price
    sns.barplot(x=gamesdf["positive_ratio"], y=df["price_final"])
    plt.show()
